<template>
      ...
      <div class="inbox-page">
        ...
        <div class="inbox-wrap card">
          ...
          <div class="inbox-main">
            ...
            <div v-if="!currentMessages.length" class="empty-state">No messages here.</div>
  ...
  const currentTabLabel = computed(() => {
    const map: Record<string,string> = { recv: 'Received messages', sent: 'Sent messages', read: 'Read messages' }
    return map[activeTab.value]
  })
  
  interface Message {
    id: string; fromName: string; preview: string; time: string; read: boolean; tab: string
  }
  
  const messages = ref<Message[]>([
    { id:'1', fromName:'Sigma boy', preview:"i kno ure there, you little mint enjoyer, little pingu lover, dm me back or else grrrrrrrrrrrr...", time:'1s ago', read:false, tab:'recv' },
    { id:'2', fromName:'Sigma boy', preview:'hllo sexy i heard u like linux', time:'4s ago', read:false, tab:'recv' },
  ])
  
  const currentMessages = computed(() => messages.value.filter(m => m.tab === activeTab.value))
  </script>
  
  <style scoped>
  .inbox-page { max-width: 860px; margin: 0 auto; }
  .page-title { font-size: 20px; font-weight: 800; text-align: center; margin-bottom: 20px; }
  
  .inbox-wrap { display: flex; overflow: hidden; min-height: 400px; }
  ...
  .inbox-section-title {
    ...
    padding: 16px; border-bottom: 1.5px solid var(--blue-pale);
  }
  ...
  .msg-row:hover  { background: var(--blue-soft); }
  ...
  .msg-type {
    ...
    color: var(--navy); border-color: var(--navy-light) !important;
