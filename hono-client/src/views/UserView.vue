    ...
    <div class="user-page card">
      ...
      <div class="avatar-col">
        <div class="profile-avatar">
          ...
          <span v-else class="av-initials">{{ user.username.slice(0,2).toUpperCase() }}</span>
        </div>
      </div>
      ...
      <div class="info-card dashed">
        <div class="info-top">
          <div class="info-left">
            ...
            <blockquote v-if="user.quote" class="user-quote">"{{ user.quote }}"</blockquote>
          </div>

          <div class="info-actions">
            <button v-if="!isOwnProfile" class="pill action-btn msg-btn dashed" @click="$router.push(`/inbox/new?to=${user.id}`)">msg</button>
          </div>
          ...
          <div class="info-stats">
            ...
            <div class="stat-follow-row">
              ...
              <button v-if="!isOwnProfile" class="pill follow-btn" :class="{ following: isFollowing }" @click="toggleFollow">
...
.info-left { flex: 1; display: flex; flex-direction: column; gap: 5px; }
...
.role-badge.user    { color: var(--purple); }
.role-badge.admin   { color: var(--red); }
.role-badge.support { color: var(--orange); }

.pronouns { font-size: 13px; color: var(--text-muted); }
...
.info-actions { display: flex; flex-direction: column; align-items: center; padding-top: 4px; }
...
.follow-btn.following { background: var(--border); color: var(--text-muted); border-color: var(--border) !important; }
...
.divider { border-top: 1.5px dashed var(--border); margin: 16px 0; }
.bio { font-size: 14px; color: var(--text); line-height: 1.6; }
</style>