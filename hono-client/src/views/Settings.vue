<template>
    <AppLayout>
      <div class="settings-page">
        <div class="settings-header">
          ...
          <RouterLink to="/account" class="account-link">(Account customization here) (link)</RouterLink>
        </div>
        ...
        <div class="sections card">
          ...
          <SettingsSection title="Credentials" :open="true" accent="default">
            <div class="creds-form">
              <label class="field-row">
                ...
                <input v-model="creds.current" type="password" class="field-input" />
              </label>
              <label class="field-row">
                <span class="field-label">Desired password:</span>
                <input v-model="creds.desired" type="password" class="field-input" />
              </label>
              <label class="field-row">
                <span class="field-label">Confirm password:</span>
                <input v-model="creds.confirm" type="password" class="field-input" />
              </label>
              <div class="field-row">
                <span class="field-label" />
                <button class="pill confirm-btn" @click="changePassword">Confirm password change</button>
              </div>
          ...
          <SettingsSection v-if="authStore.isAdmin || isSupport" title="Tickets" accent="orange">
            <label class="toggle-row">
  ...
  const inbox   = ref({ allowAll: false, notified: authStore.user?.notified ?? true })
  
  async function changePassword() {
    pwMsg.value = ''
    if (!creds.value.current) { pwMsg.value = 'Enter your current password.'; pwMsgType.value = 'err'; return }
    if (creds.value.desired.length < 8) { pwMsg.value = 'New password must be at least 8 characters.'; pwMsgType.value = 'err'; return }
    if (creds.value.desired !== creds.value.confirm) { pwMsg.value = 'Passwords do not match.'; pwMsgType.value = 'err'; return }
    // TODO: call API
  ...
  .settings-title { font-size: 32px; font-weight: 800; }
  .account-link { font-size: 13px; color: var(--purple); }
  .account-link:hover { text-decoration: underline; }
  
  .sections { padding: 0; overflow: hidden; }
  .section-divider { border-top: 1.5px dashed var(--border); }
  
  /* credentials form */
  .creds-form { display: flex; flex-direction: column; gap: 10px; }
  .field-row { display: flex; align-items: center; gap: 12px; }
  .field-label { width: 155px; font-size: 13px; font-weight: 600; flex-shrink: 0; text-align: right; }
  .field-input {
    flex: 1;
  ...
  .confirm-btn:hover { background: var(--blue-soft); }
  
  .pw-msg { font-size: 13px; padding-left: 167px; }
  ...
  .toggle-row:not(:last-child) { border-bottom: 1px solid var(--blue-pale); padding-bottom: 8px; margin-bottom: 4px; }
  </style>